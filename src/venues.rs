...
pub async fn add(arg_venue: Json<Venue>, user: Claims, tdb: TourDB) -> Result<Created<Json<Uuid>>> {
...
#[get("/<venueid>")]
pub async fn get(venueid: Uuid, tdb: TourDB) -> Result<Json<Vec<Venue>>, NotFound<String>> {
    let results = tdb
        .run(move |connection| {
            crate::schema::venue::dsl::venue
...
pub async fn delete(
    ...
    let results = tdb
        .run(move |connection| {
            diesel::delete(crate::schema::venue::dsl::venue.filter(id.eq(venueid)))
                .execute(connection)
        })
        .await;
    if results.unwrap() == 1 {
